"""Create the Step1X-3D serverless template + endpoint on RunPod.
...
NAME = "paradox-step1x3d"
IMAGE = os.getenv(
    "STEP1X3D_IMAGE", "ghcr.io/satanexist/paradox_worker:step1x3d-latest"
)
GPU_TYPE_IDS = [
...
def main() -> int:
    ...
    shared = {
        k: v
        for k, v in donor_env.items()
        if k.startswith("R2_") or k in ("HF_TOKEN", "HUGGINGFACE_TOKEN")
    }
    if not shared:
    ...
    template_body = {
        ...
        "readme": "Step1X-3D N4 geometry worker: image-to-watertight mesh (no texture bake).",
        "env": env,
    }
    endpoint_body = {
        "name": NAME,
    ...
    print(f"add to .env:  RUNPOD_ENDPOINT_ID_STEP1X3D={endpoint_id}")
    print(f"add to .env:  RUNPOD_STEP1X3D_TEMPLATE_ID={template_id}")
    return 0
