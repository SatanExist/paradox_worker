...
### huggingface_hub 1.x ломает пересборку старых образов (2026-08-20)

Симптом на сборке: образ собирается, а любой импорт `transformers` падает с

```
ImportError: huggingface-hub>=0.34.0,<1.0 is required for a normal functioning
of this module, but found huggingface-hub==1.28.0
```

Причина: во всех наших Dockerfile стоит просто `pip install huggingface_hub`, а теперь это резолвится в ветку **1.x**, которую `transformers` отказывается принимать. Раньше рецепт работал, потому что latest был `<1.0`.

**Лечение:** ставить `"huggingface_hub>=0.34,<1.0"` и **повторять пин последним шагом** — `diffusers`, `accelerate` и MoGe умеют утянуть его обратно наверх.

Важное следствие: это ждёт и `Dockerfile.trellis2` / `.hi3dgen` / `.mvadapter` при следующей пересборке. Собранные образы в GHCR не затронуты, ломается именно новая сборка.
